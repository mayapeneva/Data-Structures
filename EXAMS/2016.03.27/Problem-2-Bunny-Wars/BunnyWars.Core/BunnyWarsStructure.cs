namespace BunnyWars.Core
{
    ...
    public class BunnyWarsStructure : IBunnyWarsStructure
    {
        ...
        private readonly OrderedDictionary<int, LinkedList<Bunny>[]> bunniesInTeamsByRoom =
            new OrderedDictionary<int, LinkedList<Bunny>[]>();

        private readonly OrderedSet<Bunny> bunniesOrderedBySuffix =
            new OrderedSet<Bunny>(SuffixComparator);

        private readonly OrderedSet<int> roomsById =
            new OrderedSet<int>();

        private readonly OrderedSet<Bunny>[] bunniesByTeam =
            new OrderedSet<Bunny>[TeamCount];

        public int BunnyCount => this.bunniesByName.Count;
        public int RoomCount => this.roomsById.Count;
        ...
        public void AddBunny(string bunnyName, int teamId, int roomId)
        {
            ...
            if (this.bunniesByTeam[teamId] == null)
            {
                this.bunniesByTeam[teamId] = new OrderedSet<Bunny>(
                    (b1, b2) => String.CompareOrdinal(b1.Name, b2.Name));
            }
        ...
        {
            if (!this.bunniesInTeamsByRoom.TryGetValue(roomId, out var teamsInRoom))
            {
                throw new ArgumentException("Room does not exist");
            }
        ...
        {
            if (!this.bunniesByName.TryGetValue(bunnyName, out var bunny))
            {
                throw new ArgumentException("Bunny does not exist");
            }
